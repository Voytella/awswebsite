...
# hard-coded title of website
HEADER = "Auto Deploy Test"
...
def lambda_handler(event, context):
    ...
    if ROOT_DIR not in [content["Key"] for content in s3.list_objects_v2(Bucket=BUCKET)['Contents']]: 
        s3.put_object(Bucket=BUCKET, Key=ROOT_DIR)

    # grab all the objects in the root directory of the bucket
    response = s3.list_objects_v2(Bucket=BUCKET, Prefix=ROOT_DIR)
    
    # exit with error if ROOT_DIR is empty
    if not response:
       return {
            'statusCode': 200,
            'body': json.dumps(f'{ROOT_DIR} is empty.')
        } 
    ...
    newIndexHTML.write(bytes(f'<h1>{HEADER}</h1>\n', 'utf-8'))
